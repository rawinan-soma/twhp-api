...
|---|---|---|---|
| Factory | Its own current-fiscal participation | Register; log in after validation; create/update Enrollment; create Cover; save/edit/submit Answers; redo a hard-rejected Answer; read own score. It has **no** response to a settled score change (ADR-0012). | `factoryGuard`; `src/routes/factories/**`; `factoryService`, `enrollService`, `answerService`, `scoreService` |
