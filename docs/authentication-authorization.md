...
- **Proxy trust — Verified configuration / deployment Unknown:** repository Nginx appends `X-Forwarded-For`; Elysia no longer logs that header and does not use it for authorization/rate limiting. No trusted-proxy list or real-IP chain is configured for the API location. Client-supplied forwarded values can make logs ambiguous unless an upstream strips/normalizes them.
- **Logging — Verified:** request logging records method, path without query string, route template, status, duration and JWT `sub` as `userId`. It records no URL query, headers, cookies, IP, user agent or body; pino `redact` masks authorization, cookie, password, OTP, token and email/recipient keys as a safety net. Unexpected-error lines still carry the raw error message, which for a failed Drizzle query includes the SQL and its parameters. Clients receive a generic unexpected-error body.
