...
- **Implementation:** `evaluator-review.ts:getAnswers`, gated on `ReviewerScope.kind === "province"`; `resolveProvincialOfficer` (level `ODPC`, so all five `QuestionCategory` values are in scope); `latestCoverLogFor` for the status gate; `src/routes/provincialOfficers/covers/[coverId]/answers/index.ts`. Decision recorded in [ADR-0013](adr/0013-province-scoped-read-only-cover-review.md).
