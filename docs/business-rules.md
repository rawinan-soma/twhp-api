...
- **Implementation:** `answer.ts:58-219,403-603,666-967`; `answer-file-update.ts:3-65`; `schema/answer.ts:14-74`.
- **Inputs/conditions:** effective choice, Question `special`, new/existing PDF files.
- **Result:** validated filenames stored.
- **Edges/failure:** on initial `0`/`n/a`, supplied files pass DTO validation but are silently ignored. `special=3` edit/redo clears non-selected groups; uploads for those implicitly cleared rows are ignored. Other specials preserve all old groups even on lowering. For non-special `0`/`n/a`, update/redo may preserve or upload evidence although initial create ignores it. MinIO changes happen before DB commit. PATCH supports optional `delete_file_<row>_<slot>` flags while the latest Answer status is `in_review`. Deletion eligibility is independent of `Question.special`; the projected evidence must still satisfy the existing choice matrix. Omission preserves a stored file. Strict explicit deletions run concurrently; one can succeed or remain in flight when another fails because `Promise.all` cannot cancel MinIO work. Later upload or database failure can occur after an explicit removal, leaving dangling filenames; concurrently completed uploads can become orphans. Legacy implicit clearing and replacement cleanup use best-effort deletion, which catches and logs deletion failures and permits the DB update/success, so an old object can remain orphaned.
- **Failure behavior:** missing required evidence 400. Strict explicit-delete or upload failure returns 500 before the DB update; a database failure also returns 500. Best-effort cleanup failure does not fail the update. There is no automatic compensation, recovery, or reconciliation; operators must reconcile affected Answer columns and MinIO objects without reproducing object names or presigned URLs in operational reports.
...
- **Edges/failure:** failed DB after upload leaves orphans; delete-before-update may break references; finalize delete-before-transaction can lose evidence on DB failure. Concurrent strict Answer deletions are not cancellable: one MinIO delete may succeed or finish after another fails. A later upload failure can occur after explicit deletion, and another concurrent upload can already have succeeded. These strict/upload failures return 500 with no DB update while stored filenames can reference removed objects and completed uploads can be orphaned. Legacy implicit/replacement cleanup instead catches and logs deletion failures, permits DB success, and can leave an old object orphaned. There is no automatic recovery, outbox, compensation, reconciliation, lock, version, or idempotency key; operator reconciliation is required.
