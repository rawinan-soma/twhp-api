...
- **Severity:** Medium
- **Category:** Confirmed debt
- **Evidence:** Bun and the former `latest` dependencies (`elysia`, `bun-types`) have been pinned to exact versions since 2026-09-25. API/worker packaging differs (API runs source/autoload; worker is a compiled platform-specific binary). `APP_PORT` is configurable but Docker/Nginx/healthchecks assume 3000. Drizzle config imports the full eager app config but directly reads `process.env.DATABASE_URL`; seed does the same. No value-free `.env.example` exists. Accepted ADRs, `CONTEXT.md`, historical intent files, static OpenAPI, and implementation contain conflicting claims about finalization, choice provenance, file deletion, and email fields.
