...
- **Update 2026-09-21:** ADR-0014 settled the Gold `special` set, then corrected it the same day on the maintainer's word: `gold` gates on `special == 1` only, the `special=2` Questions gate only the new `consec-gold` tier, and `special=3` gates no tier. Standard auto-credit acceptance, N/A eligibility, assignment meaning, and the absent transition matrix are unchanged and still required before handover.
