...
**อัปเดต 3 ก.ย. 2026 — หลังจากสแกนครั้งนี้มี path เพิ่ม 1 ตัว:**
`GET /twhp/api/provincialOfficers/covers/:coverId/answers` (ดู § 6) ตัวเลข `path เพิ่ม / หาย = 0`
ด้านบนเป็นผลของการสแกนวันที่ 26 ส.ค. เท่านั้น
...
9. **ทำหน้าดูคำตอบสำหรับ Provincial Officer** — endpoint ใหม่ใน § 6
   ใช้ response type เดียวกับหน้าผู้ประเมิน แต่ห้าม reuse component ที่มีปุ่มตัดสิน
...
## § 6 — Provincial Officer เปิดดูคำตอบของโรงงานได้แล้ว 🟢 ADDITIVE

เพิ่ม 3 ก.ย. 2026 · ADR-0013 · ไม่กระทบ endpoint เดิม

### `GET /twhp/api/provincialOfficers/covers/:coverId/answers`

เจ้าหน้าที่จังหวัดเปิดดูคำตอบรายข้อของฝาประเมินในจังหวัดตัวเองได้ — **อ่านอย่างเดียว**
response เป็น `AnswerViewSchema` **ตัวเดียวกับ** `GET /evaluators/covers/:coverId/answers`
จึง generate type ตัวเดิมมาใช้ซ้ำได้เลย แต่**ค่าที่ได้ไม่เหมือนกัน** ตามสถานะของฝา

| สถานะฝา | Officer เห็นอะไร |
|---|---|
| `in_progress` | **404 `{ message: "cover not found" }`** — เหมือนฝาของจังหวัดอื่นทุกประการ |
| `in_review` | คำตอบ + ไฟล์หลักฐาน + ใบรับรองมาตรฐาน แต่ `verdictChoice` กับ `description` ถูกบังคับเป็น `null` และ `status` รายข้อถูกบังคับเป็น `"in_review"` ทุกข้อ |
| `finished` | เห็นเท่ากับที่ผู้ประเมินเห็นทุกอย่าง |

### สามอย่างที่พลาดกันบ่อย

1. **`status` ตอน `in_review` ไม่ใช่ค่าจริง** — ทุกข้อจะเป็น `"in_review"` เหมือนกันหมด
   แม้ผู้ประเมินตัดสินไปแล้ว อย่าเอาไปคำนวณ progress bar หรือ badge "ตรวจแล้ว x/y ข้อ"
   ตัวเลขจะนิ่งอยู่ที่ 0 จนกว่าฝาจะ `finished` แล้วกระโดดเต็มทีเดียว — เป็นพฤติกรรมที่ตั้งใจ
2. **404 ไม่ได้แปลว่า "ไม่มีสิทธิ์"** — ฝาที่โรงงานยังไม่ส่ง กับฝาของจังหวัดอื่น
   คืนข้อความเดียวกันโดยตั้งใจ (ไม่ยืนยันว่าฝานั้นมีอยู่จริง) ให้แสดงว่า "ไม่พบฝาประเมิน"
   **อย่า**แสดงว่า "คุณไม่มีสิทธิ์เข้าถึง" เพราะจะกลายเป็นการบอกว่าฝานั้นมีอยู่
3. **ใบรับรองมาตรฐานไม่ถูกซ่อน** ทั้งสองสถานะ — เป็นเอกสารที่โรงงานส่งเอง ไม่ใช่ผลการตัดสิน

### ไม่มี route สำหรับเขียน

`provincialOfficers/**` ไม่มี verdict-save และไม่มี finalize และจะไม่มี
ถ้า reuse component ของหน้าผู้ประเมิน **ต้องซ่อนปุ่มตัดสิน/ปุ่ม finalize ให้หมด**
ไม่ใช่แค่ disable — ยิงไปก็ไม่มี endpoint รับ
