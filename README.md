...
enrollment, workplace-health assessment with evidence upload, hierarchical and geographically
scoped evaluator review, finalization, on-demand score/grade calculation, and email notification.

Bun + ElysiaJS + TypeBox, PostgreSQL (Drizzle), Redis/BullMQ, MinIO, Nodemailer. Two processes: the
HTTP API and the email worker.
...
| Queue / cache | Redis 7 + BullMQ (`email` queue, daily 08:30 Bangkok repeat job), delivered via Nodemailer |
...
20 test files. 10 files are isolated; 10 are PostgreSQL integration tests.

```bash
# Isolated, safe to run anywhere. 224 pass / 0 fail as of 2026-09-25 (Bun 1.3.6).
