...
- `grade` — the [[Grade]] (`gold`/`silver`/`certificate`/`joined`), present **only when `coverStatus` is `finished`** (otherwise `null`). Read from the stored `Awards` row (the score is recomputed on demand) — this is the retrieval path for a Grade after the finalize email.
...
### Grade
The award tier derived from a `finished` Cover's scores. One of `gold`, `silver`, `certificate`, `joined`. Computed once at finalize and **stored** in `Awards` (never recomputed), evaluated **strictly top-down — the first tier whose conditions all pass** (the lower tiers are *floors*, not bands, so the ordering is load-bearing):
