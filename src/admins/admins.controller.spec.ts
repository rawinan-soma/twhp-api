...
describe('Admins', () => {
  ...
  beforeEach(async () => {
    ...
    const evalOdpcId = (await prisma.accounts.findFirst({
      where: { username: 'odpc6' },
    }))!.id;

    const evalDohId = (await prisma.accounts.findFirst({
      where: { username: 'doh6' },
    }))!.id;

    const evalMentalId = (await prisma.accounts.findFirst({
      where: { username: 'mental6' },
    }))!.id;

    await prisma.enrolls.create({
      data: {
        factory: { connect: { account_id: factoryId } },
        eval_doh: { connect: { account_id: evalDohId } },
        eval_mental: { connect: { account_id: evalMentalId } },
        eval_odpc: { connect: { account_id: evalOdpcId } },
        employee_th_m: 25,
        employee_mm_m: 10,
        employee_kh_m: 5,
        employee_la_m: 3,
  ...
  it('should get all enrolls data', async () => {
    await agent.get('/admins/enrolls').expect(200);
  });
});
